...
@Component({
  ...
  template: `
    <div class="min-h-screen flex bg-surface-50 relative w-full">
      @if (sidebarOpen()) {
        <div class="fixed inset-0 bg-surface-900/50 z-20 md:hidden" (click)="toggleSidebar()"></div>
      }
      <app-sidebar class="shrink-0 fixed inset-y-0 left-0 z-30 transition-transform duration-300 md:sticky md:top-0 md:h-screen md:translate-x-0"
                   [class.-translate-x-full]="!sidebarOpen()"
                   [class.translate-x-0]="sidebarOpen()" />
      <main class="flex-1 p-4 sm:p-6 flex flex-col w-full min-w-0">
